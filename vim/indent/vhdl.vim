...
setlocal indentkeys+==~report,=~severity
...
" Align port ) with port (
if !exists("g:vhdl_indent_portparensalign")
  let g:vhdl_indent_portparensalign = 1
endif

" Align assert <condition> report "string" severity <level>
if !exists("g:vhdl_indent_report")
  let g:vhdl_indent_report = 0
endif
...
function GetVHDLindent()
  ...
  if g:vhdl_indent_report
    if curs =~? '\<report\>' && prevs =~? '\<assert\>'
      return ind + &sw
    endif
    if curs =~? '\<severity\>' && prevs =~? '\<report\>'
      return ind
    endif
  endif
  ...
  if (curs =~ '^\s*)' || curs =~? '^\s*\%(\<\%(procedure\|generic\|map\|port\)\>.*\)\@<!\w\+\s*\w*\s*\%(=>\s*\S\+\|:[^=]\@=\s*\%(\%(in\|out\|inout\|buffer\|linkage\)\>\|\w\+\s\+:=\)\)') && (prevs =~? s:NC.'\<\%(procedure\s\+\S\+\|generic\|map\|port\)\s*(\%(\s*\w\)\=' || (ps =~? s:NC.'\<\%(procedure\|generic\|map\|port\)'.s:ES && prevs =~ '^\s*('))
    ...
    if curs =~ '^\s*)'
      if g:vhdl_indent_portparensalign = 1
